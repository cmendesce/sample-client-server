...
public abstract class BackendServiceTemplate {
    ...
    public <P> ResilienceModuleMetrics doHttpRequest(RequestModel requestModel) {
        ...
        while (successfulCalls < requestModel.getSuccessfulRequests() && requestModel.getMaxRequests() > metrics.getTotalRequests()) {
            var supplier = decorate(() -> this.sendRequest(requestModel.getTargetUrl()));
